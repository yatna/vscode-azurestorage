...
class BaseEditor {
    ...
    showEditor(context, sizeLimit /* in Megabytes */) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const resourceName = yield this.getResourceName(context);
            this.appendLineToOutput(localize_1.localize('opening', 'Opening "{0}"...', fileName), { resourceName: resourceName });
    ...
    appendLineToOutput(value, options) {
        extensionVariables_1.ext.outputChannel.appendLog(value, options);
    ...
    updateRemote(context, doc) {
        return __awaiter(this, void 0, void 0, function* () {
            const filename = yield this.getFilename(context);
            const resourceName = yield this.getResourceName(context);
            this.appendLineToOutput(localize_1.localize('updating', 'Updating "{0}" ...', filename), { resourceName: resourceName });
            const updatedData = yield this.updateData(context, doc.getText());
            this.appendLineToOutput(localize_1.localize('done', 'Updated "{0}".', filename), { resourceName: resourceName });
            if (doc.isClosed !== true) {
                const visibleDocument = vscode.window.visibleTextEditors.find((ed) => ed.document === doc);
                if (visibleDocument) {
                    yield this.updateEditor(updatedData, visibleDocument);
                }
            }
        });
    }
