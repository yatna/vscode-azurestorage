...
export class BlobContainerGroupTreeItem extends AzureParentTreeItem<IStorageRoot> {
    ...
    public get label(): string { return `Blob Containers${this.active ? '' : ' (stopped)'}`; }
    public get contextValue(): string { return `${BlobContainerGroupTreeItem.contextValue}${this.active ? '' : 'Stopped'}`; }
